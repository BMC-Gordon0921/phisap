...
## 分支说明
**这个分支是`no-streaming`**，即去掉了流式传输的分支。如果您在使用主分支时遇到了server报错，可以尝试使用这个分支。与本分支对应的预编译server可以在release页面找到(v0.0.1-beta)。注意：该分支目前仅适用安卓版本8.0以下的设备，如为8.0以上的安卓版本，请使用主分支。
...
## 在早期adb版本中导致程序进程异常卡死无返回信息的问题
在早期版本的adb中不支持 adb reverse 命令，请安装（替换）1.0.41版本的adb。对应的adb文件已经在 no-streaming 分支和主分支上传。
